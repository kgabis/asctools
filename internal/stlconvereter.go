...
func (elevationMap *ElevationMap) WriteSTL(writer *bufio.Writer) error {
	header := make([]byte, 80)
	copy(header, []byte("STL binary file generated by Go"))

	triangles := createTriangles(elevationMap, 0.0)
	defer writer.Flush()
...
func addTriangles(triangles []Triangle, v1 Vector3, v2 Vector3, v3 Vector3) []Triangle {
	v1floor := Vector3{v1.X, v1.Y, 0}
	v2floor := Vector3{v2.X, v2.Y, 0}
	v3floor := Vector3{v3.X, v3.Y, 0}

	triangles = addTriangle(triangles, v1, v2, v3)
	triangles = addTriangle(triangles, v1floor, v3floor, v2floor)
		...
		for x := 0; x < len(elevationData[y])-1; x++ {
			...
			if e3 < 0 || e2 < 0 {
				if v1.Z >= 0 && v4.Z >= 0 && v3.Z >= 0 {
					triangles = addTriangles(triangles, v1, v4, v3)
				}
				if v1.Z >= 0 && v2.Z >= 0 && v4.Z >= 0 {
					triangles = addTriangles(triangles, v1, v2, v4)
				}
			} else {
				if v1.Z >= 0 && v2.Z >= 0 && v3.Z >= 0 {
					triangles = addTriangles(triangles, v1, v2, v3)
				}
				if v2.Z >= 0 && v4.Z >= 0 && v3.Z >= 0 {
					triangles = addTriangles(triangles, v2, v4, v3)
				}
			}
